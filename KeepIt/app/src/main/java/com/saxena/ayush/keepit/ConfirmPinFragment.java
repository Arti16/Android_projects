...
public class ConfirmPinFragment extends Fragment implements View.OnClickListener {
    Button btn1,btn2,btn3,btn4,btn5,btn6,btn7,btn8,btn9,btn0,btnDel;
    RelativeLayout layout;
    TextView pin1,pin2,pin3,pin4,pin5,pin6,label;
    String tempPin="";
    int pointer;
    @Nullable
    @Override
    ...
    void init(View v)
    {
        ...
        label=(TextView)v.findViewById(R.id.label_setpin);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        label.setText("Re Enter the Passcode");
    ...
    public void onClick(View view) {
        ...
        if(pointer==6)
        {
            if(Objects.equals(FirstSetUp.loginPin, tempPin))
            {
                FirstSetUp.confirmed=true;
                FragmentSuccess frag=new FragmentSuccess();
                getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragment_container,frag).commit();
            }
            else
            {
                Snackbar snack=Snackbar.make(layout,"Passcode do not match..Retry",Snackbar.LENGTH_LONG);
                snack.show();
                pointer=0;
                tempPin="";
                for(int i=1;i<=6;i++)
